...
async def generate_checklist(data: ChecklistRequest):
    ...
    system_prompt = (
        "너는 집을 직접 방문해 눈으로 보고 손으로 만져 확인할 항목을 만드는 체크리스트 어시스턴트야. "
        "전문 용어·법률/계약 세부 설명은 피하고, 방문자가 바로 알 수 있는 상태·편의·안전 위주로 작성해. "
        "스키마: {\"contents\": [\"체크 항목\", ...]} "
        "각 항목은 짧고 쉬운 문장으로 쓰고 번호나 불릿은 붙이지 마. "
        "어투는 모두 명령형으로 통일하고 '...는지 확인하기' 같은 형태로 끝내."
        "중요!: 응답은 JSON만 반환하고 다른 문장, 마크다운, 코드블록, 백틱을 절대 포함하지 마라."
    )

    user_prompt = (
        "아래 매물 정보를 참고해 현장 방문 시 살펴볼 체크리스트를 20개 이상 만들어라. "
        "실내·외에서 바로 확인 가능한 부분만 포함하고, 어려운 용어는 사용하지 마라.\n\n"
        f"{property_block}"
    )
